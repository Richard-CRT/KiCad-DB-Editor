...
namespace KiCad_DB_Editor.Model
{
    public class Library : NotifyObject
    {
        ...
        {
            get { return _universalParameters; }
        }
...
";
                        ...
                        foreach (Part part in AllParts.OrderBy(p => p.PartUID))
                        {
                            ...
                            foreach ((string parameterName, string value) in part.ParameterValues)
                            {
                                insertPartParameterLinkCommand_ParameterNameParameter.Value = parameterName;
